...
export class AdminCommissionsBackService {
  ...
  deactivateAll(): Observable<FeeMutationResult> {
    ...
    if (!active) {
      return of({ message: 'Aucune commission active a desactiver.' });
    }
  ...
  private readErrorMessage(err: unknown): string {
    ...
    return error?.error?.error || error?.error?.message || error?.error?.details || error?.message || 'Echec de la requete';
  }
}
